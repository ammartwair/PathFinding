...
def dfs(draw, start_node, end_node, nodes):
...
def bfs(draw, start_node, end_node, nodes):
...
def draw_board(win, rows, width):
    ...
    for x in range(1, rows):
        pygame.draw.line(win, LINE_COLOR, (gap*x, 0), (gap*x, width), 2)
...
dfsbtn = pygame.draw.rect(WIN, START , (BTNX,BTNY,BTNW,BTNH))
bfsbtn = pygame.draw.rect(WIN, VISITED , (BTN1X,BTNY,BTNW,BTNH))
astarbtn = pygame.draw.rect(WIN, END , (BTN2X,BTNY,BTNW,BTNH))
text = font.render("DFS" , True , BLACK)
text1 = font.render("BFS", True , BLACK)
text2 = font.render("AStar", True , BLACK)
WIN.blit(text,(120,630))
WIN.blit(text1,(270,630))
WIN.blit(text2,(420,630))
...
def main(win, width, rows):
    ...
    while running:
        for event in pygame.event.get():
            ...
            elif dfsbtn.collidepoint(pygame.mouse.get_pos()):
                if pygame.mouse.get_pressed()[0]:
                    dfs(lambda: draw(win, rows, width, nodes), start, end, nodes)
            elif astarbtn.collidepoint(pygame.mouse.get_pos()):
                if pygame.mouse.get_pressed()[0]:
                    a_star(lambda: draw(win, rows, width, nodes), start, end, nodes)
            elif bfsbtn.collidepoint(pygame.mouse.get_pos()):
                if pygame.mouse.get_pressed()[0]:
                    bfs(lambda: draw(win, rows, width, nodes), start, end, nodes)
